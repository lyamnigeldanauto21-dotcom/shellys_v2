...
local function RebuildActiveLabels(categoryKey, labelTag)
    ...
    for targetObj, _ in pairs(RenderTracker[categoryKey]) do
        ...
        if billboard then
            ...
            if textLabel then
                -- Single synchronized total update completely wipes clean trailing random text rows
...
local function FilterAndRegister(descendant)
    ...
    -- Accurate character architecture evaluation for hostile models
    ...
    -- Filter static environment models out, target core interactive structures
    ...
    -- Supply Crates / Interaction Consumables
...
local function InterceptSkillNetwork()
    -- Direct internal network listeners bypass touch simulation problems completely
        ...
        if obj:IsA("RemoteEvent") and (string.find(obj.Name, "Skill") or string.find(obj.Name, "Minigame")) then
            obj.OnClientEvent:Connect(function(...)
                if Flags.AutoSkillCheck then
                    -- Fires back a perfect validation signal directly to the extraction handler
                    task.wait(0.05)
                    obj:FireServer(true)
                end
            end)
...
-- Render Options Out Inside Tab Columns
